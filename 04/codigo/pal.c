...
void InsereNaTabela (Hash *t, Info *h, char * palavra) {
  ...
  hxx = Disperse(palavra, h->m);
  if (t[hxx]==NULL) {
    t[hxx] = AlocaNo(palavra);
    h->numPalavras++;
    h->num_pal[hxx]=1;
    return;
  } else if ((cmp = strcmp(palavra,t[hxx]->palavra)) >= 0) {
    if (cmp==0) {
      t[hxx]->ocorrencias++;
      return;
    } else if(cmp>0) {
    ...
    while (x->prox != NULL) {
      cmp = strcmp(palavra,x->prox->palavra);
      if (cmp==0) {
        x->prox->ocorrencias++;
        return;
      }
      else if (cmp>0) {
        AcrescentaNo(AlocaNo(palavra), x);
        h->numPalavras++;
        h->num_pal[hxx]++;
        return;
      }
