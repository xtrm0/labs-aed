 ...
 *
 *  Return value:
 *    Returns 0 upon sucess, 1 in case the item is NULL, and 2 in
                           ...
                           (Item item1, Item item2))
{
  ...
  LinkedList * new, *previous, *current;

  /* Cria elemento  e inicializa*/
  /* Memory allocation                                            */
  new = (LinkedList *) malloc(sizeof(LinkedList));
  ...
  /*Se ja houver elementos... tenho q comparar ! */
  previous = NULL;
  current  = first;
  while( current != NULL ){
	  if( comparisonItemFnt( current->this, new->this) > 0 ){
		  /*INSERE ELEMENTO*/
		  if(previous == NULL ){ /* Se inserir Ã  cabeca: todos os elementos da lista maiores que o novo */
			  new->next = current;
			  first = new;
		  }else{
			  new -> next = current;
  ...
  /* Se nao inserio em nenhum sitio, insere no final!! */
  previous -> next = new ;
  return first;
}
